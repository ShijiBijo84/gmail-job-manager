...
# Copy pre-built client dist (built locally, not in Docker to avoid tailwindcss native binary issues)
COPY --chown=appuser:appuser packages/client/dist ./packages/client/dist
COPY --chown=appuser:appuser packages/client/package.json ./packages/client/package.json
